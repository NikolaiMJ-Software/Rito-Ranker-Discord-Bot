...
class Accounts(commands.Cog):
    ...
    @app_commands.describe(riot_id="Riot ID like GameName#TAG", platform="Platform like EUW1 / EUN1 / NA1")
    ...
    async def link(self, interaction: discord.Interaction, riot_id: str, platform: app_commands.Choice[str]):
        ...
        try:
            # Account-V1 can query any account; using europe is fine for v1 simplicity. :contentReference[oaicite:2]{index=2}
            puuid, game_name, tag_line = await get_puuid_by_riot_id(RIOT_API_KEY, riot_id, region_cluster="europe")
